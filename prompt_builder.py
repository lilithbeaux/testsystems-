...
def build_skills_system_prompt(
    ...
    ``priority_categories`` inverts the logic: when set, ONLY the listed
    categories get full descriptions and everything else is compacted to
    names-only.  Mutually exclusive with ``compact_categories`` — if both are
    set, ``priority_categories`` wins.
    """
    ...
    # their parent.
    # priority_categories inverts the logic: ONLY the listed categories get
    # full descriptions; everything else is compacted to names-only.
    # This replaces compact_categories when both are set.
    _effective_compact = compact_categories
    if priority_categories is not None:
        _effective_compact = frozenset(
            cat for cat in skills_by_category
            if cat.split("/", 1)[0] not in priority_categories
        )

    demoted = frozenset(
        cat for cat in skills_by_category
        if cat.split("/", 1)[0] in (_effective_compact or frozenset())
    )

    hidden_note = ""
    if demoted:
        hidden_note = (
            "\n(Categories marked [names only] are outside the current "
